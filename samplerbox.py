...
def MidiCallback(mididev, message, time_stamp):
    ...
    for outport in gv.outports:
        if mididev != gv.outports[outport][0]:  # don't return to sender
            ...
            #next is currently hypothetical as standard rtmidi ignores sysex input
            #if ( message[0] == 0xF0 ):
            #    print "got x-%s-x" %message
            #    gv.outports[outport][1].send_sysex(message[1:len(message)-1])
            #    print "snt x-%s-x" %message[1:len(message)-1]
            #elif ( len(message) == 1 ):
            if ( len(message) == 1 ):
                gv.outports[outport][1].send_raw(message[0])
            elif ( len(message) == 2 ):
                gv.outports[outport][1].send_raw(message[0], message[1])
            else:
                gv.outports[outport][1].send_raw(message[0], message[1], message[2])
    # ---------------------------------------------------------------------------------
