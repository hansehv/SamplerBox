...
def notename2midinote(inotename,fractions):
	...
	if notename=="Ctrl":
		...
		try:
			note = notename[:len(notename)-1]
			sign = 1						# sign of the octave number
			if notename[-2:] == "-1" and fractions == 1:   # this is OK
				sign = -1
				note = notename[:len(notename)-2]
			x=notenames.index( note )
			if fractions==1:	# we have undivided semi-tones = 12-tone scale
				...
				# next statements places note C4 on 60
					...
					midinote = x + ( int(notename[-1])*sign +1) * 12
			else:			    # 24 note logic
				midinote = x + ( int(notename[-1]) -2) * 24 +12
		except:
			midinote=128
	if midinote > 127 or midinote < 0:
		print("Ignored unrecognized notename '%s'" %inotename)
		midinote = -1
	return midinote
