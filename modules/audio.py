...
def AudioCallback(outdata, frame_count, time_info, status):
	...
	for e in rmlist:
		try:
			if e.sound.stopmode in [-1, 3]:		# mode=once and backtracks have no note-off
				note = e.note + ( e.channel * gv.MTCHNOTES )
				if note in gv.playingnotes:
					if e in gv.playingnotes[note]:
						gv.playingnotes[note].remove(e)
