...
def process():
	global mode, playnote, velocity, sequence, steps
	global cycletick, steptick, stepticks, noteticks, cycleticks
		...
		if steptick == 0:
			...
			if playnote > (127-gv.stop127) and playnote < gv.stop127:	# stay within keyboard range
				for layer in gv.currlayers:
					...
					if chorus.effect:
						gv.PlaySample(playnote,playnote,voice,layvel*chorus.gain,0,0)
						gv.PlaySample(playnote,playnote,voice,layvel*chorus.gain,2,-(chorus.depth/2+1))
						gv.PlaySample(playnote,playnote,voice,layvel*chorus.gain,5,chorus.depth)
					else:
						gv.PlaySample(playnote,playnote,voice,layvel,0,0)
			else:
				playnote=-1
		...
		if playnote in gv.playingnotes:
			for m in gv.playingnotes[playnote]:
				...
				if noteticks < stepticks:			# process release sample if notes are not consecutive
					gv.PlayRelSample(m.playingrelsample(),m.playingvoice(),m.playingnote(),m.playingvolume(),m.playingretune())
		playnote = -1
...
def note_onoff(messagetype, midinote, played_velocity):
	global mode, pressed, noteon, sequence, currnote, velocity
	global steps, cycleticks, stepticks
...
def sustain(CCval,*z):	# time between note-on and note-off,
	global stepticks	# when tempo is faster it will override making it continuous
	x = ( CCval*100 ) / 127.0
	if x > 100:
		x = 100
